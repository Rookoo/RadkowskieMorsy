...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		...
		<div class="entry-meta">
			 <p class="meta-info"><span><em>
                <?php _e('Wysłane przez', 'wp-fanzone'); ?> <?php the_author_posts_link(); ?> <?php _e(' - ', 'wp-fanzone'); ?> <?php the_time( get_option( 'date_format' ) ); ?>
             </em></span></p>
		</div><!-- .entry-meta -->
	</header><!-- .entry-header -->
	...
	<div class="entry-content">
        ...
        <?php
			wp_link_pages( array(
				'before' => '<div class="page-links">' . __( 'Strona:', 'wp-fanzone' ),
				'after'  => '</div>',
			) );
		?>
        ...
        <?php if (get_theme_mod('wp_fanzone_author_bio') ) : ?>
            <div class="author-bio">        
                ...
                <div class="author-info">
                    ...
                    <h4 class="author-title"><?php _e('Wysłane przez ', 'wp-fanzone'); ?><a href="<?php echo esc_url($author_posts_url); ?>" title="<?php printf( __( 'Pokaż wpisy wysłane przez %s', 'wp-fanzone' ), get_the_author() ) ?>"><?php the_author(); ?></a></h4>
